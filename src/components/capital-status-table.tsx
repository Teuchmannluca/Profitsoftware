...
> = {
  All: { icon: BarChart3, color: "text-indigo-500", bg: "bg-indigo-50 dark:bg-indigo-950" },
  Fulfillable: { icon: Package, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-950", field: "fulfillable" },
  ...
  Researching: { icon: AlertTriangle, color: "text-violet-500", bg: "bg-violet-50 dark:bg-violet-950" },
  "Customer Damaged": { icon: AlertTriangle, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950" },
  "Warehouse Damaged": { icon: AlertTriangle, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950" },
  "Distributor Damage": { icon: AlertTriangle, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950" },
  "Carrier Damaged": { icon: AlertTriangle, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950" },
  Defective: { icon: X, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950" },
  "Customer Order": { icon: ShoppingCart, color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-950" },
  "FC Processing": { icon: Package, color: "text-sky-500", bg: "bg-sky-50 dark:bg-sky-950" },
};
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/60 bg-muted/30">
                  <th className="text-left px-4 md:px-6 py-3 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Description</th>
                  ...
                  <th className="text-right px-4 md:px-6 py-3 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Total Unit Cost</th>
                  ...
                  <th className="text-right px-4 md:px-6 py-3 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Potential Profit</th>
                  <th className="text-right px-4 md:px-6 py-3 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">% ROI</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => {
                  const isAll = row.status === "All";
                  const config = STATUS_CONFIG[row.status];
                  const Icon = config?.icon;
                  const hasField = !!config?.field;
                  const isClickable = hasField && row.units > 0;
                  ...
                  return (
                    <tr
                      ...
                      className={`${
                        isAll
                          ? "border-b-2 border-border bg-muted/30"
                          : "border-b border-border/20 hover:bg-muted/10 transition-colors"
                      } ${isClickable ? "cursor-pointer" : ""}`}
                    >
                      <td className={`px-4 md:px-6 py-3.5 ${isAll ? "font-bold" : ""}`}>
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className={`text-sm ${isAll ? "font-bold" : "font-medium"} text-foreground`}>{row.status}</span>
                          {isClickable && (
                            <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/30" />
                          )}
                        </div>
                      </td>
                      <td className={`text-right px-4 md:px-6 py-3.5 text-sm font-mono ${isAll ? "font-bold" : "font-semibold"}`}>
                        {row.units.toLocaleString("en-GB")}
                      </td>
                      <td className={`text-right px-4 md:px-6 py-3.5 text-sm font-mono ${isAll ? "font-bold" : ""}`}>
                        {fmt(row.cost)}
                      </td>
                      <td className={`text-right px-4 md:px-6 py-3.5 text-sm font-mono ${isAll ? "font-bold" : ""}`}>
                        {fmt(row.resale)}
                      </td>
                      <td className={`text-right px-4 md:px-6 py-3.5 text-sm font-mono ${isAll ? "font-bold" : "font-semibold"} ${profitColor}`}>
                        {fmt(row.profit)}
                      </td>
                      <td className={`text-right px-4 md:px-6 py-3.5 text-sm font-mono ${isAll ? "font-bold" : "font-semibold"} ${profitColor}`}>
                        {row.roi.toFixed(2)}
