-- PPC ad spend tracking + integrate into profit calculations
...
CREATE INDEX idx_ad_spend_date ON ad_spend_daily(date);


-- Update get_sales_metrics to include ad spend
...
BEGIN
  ...
  -- Ad spend for the period (AT TIME ZONE converts UTC timestamps to UK local dates)
...
-- Must drop first because the return type changed (added ad_spend column)
